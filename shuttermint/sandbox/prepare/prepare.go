...
{{$.ShielderCmd}} init --dev --root {{$d}} --index {{$i}}{{end}}
...
tmux send-keys "{{$.ShielderCmd}} run --config {{index $.KeyperDirs 0}}/config/config.toml" C-m
...
{{range $i, $d := $.KeyperDirs}}{{if $i}}
tmux split-window -h
tmux send-keys "{{$.ShielderCmd}} run --config {{$d}}/config/config.toml" C-m{{end}}{{end}}
sleep ${SHUTTERMINT_START_TIME}
