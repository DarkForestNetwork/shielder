...
{{$.ShielderCmd}} init --dev --root ${PARENT_PATH}//{{$d}} --index {{$i}}{{end}}
...
tmux send-keys "{{$.ShielderCmd}} run --config ${PARENT_PATH}//{{index $.KeyperDirs 0}}/config/config.toml" C-m
...
{{range $i, $d := $.KeyperDirs}}{{if $i}}
tmux split-window -h
tmux send-keys "{{$.ShielderCmd}} run --config ${PARENT_PATH}//{{$d}}/config/config.toml" C-m{{end}}{{end}}
sleep ${SHUTTERMINT_START_TIME}

# start keypers and make them connect to their assigned shuttermint node
{{range $i, $d := $.KeyperDirs}}
RPC_URL=$(getRPCURL {{$d}})
sed -i "s/ShielderURL = \".*/ShielderURL = \"http:\/\/${RPC_URL}\"/" {{$d}}/config.toml
tmux split-window -h
tmux send-keys "{{$.ShielderCmd}} keyper --config {{$d}}/config.toml" C-m{{end}}
