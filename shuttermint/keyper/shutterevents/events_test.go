...
func mkeq(t *testing.T, appEv abcitypes.Event, expected shielderevents.IEvent) {
	ev, err := shielderevents.MakeEvent(appEv)
	require.Nil(t, err)
...
func TestAccusation(t *testing.T) {
	...
	mkeq(t, appEv, shielderevents.Accusation{
		Eon:     eon,
		Sender:  sender,
		Accused: addresses,
	})
...
func TestMakeEventBatchConfig(t *testing.T) {
	...
	mkeq(t, appEv, shielderevents.BatchConfig{
		StartBatchIndex: 111,
		Threshold:       2,
		Keypers:         addresses,
...
func TestMakeEonStartedEvent(t *testing.T) {
	...
	mkeq(t, appEv, shielderevents.EonStarted{Eon: eon, BatchIndex: batchIndex})
}
...
func TestMakePolyCommitmentRegisteredEvent(t *testing.T) {
	...
	mkeq(t, appEv, shielderevents.PolyCommitment{
		Eon:    eon,
		Sender: sender,
		Gammas: &gammas,
	})
