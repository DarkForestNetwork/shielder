...
// tendermint event of type "shielder.encryption-key-signature-added"
...
// "shielder.new-dkg-instance".
func MakeNewDKGInstanceEvent(ev abcitypes.Event) (NewDKGInstanceEvent, error) {
	if ev.Type != "shielder.new-dkg-instance" {
		return NewDKGInstanceEvent{}, fmt.Errorf("expected event type shielder.new-dkg-instance, got %s", ev.Type)
	}
...
func MakePolyCommitmentRegisteredEvent(ev abcitypes.Event) (PolyCommitmentRegisteredEvent, error) {
	...
	if ev.Type != "shielder.poly-commitment-registered" {
...
func MakeEvent(ev abcitypes.Event) (IEvent, error) {
	...
	if ev.Type == "shielder.encryption-key-signature-added" {
	...
	if ev.Type == "shielder.decryption-signature" {
