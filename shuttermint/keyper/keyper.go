...
func (kpr *Keyper) fetchCurrentDKG(ctx context.Context) error {
	page := 1
	perPage := 50
	res, err := kpr.shmcl.TxSearch(ctx, "shielder.new-dkg-instance.Eon EXISTS", false, &page, &perPage, "desc")
	if err != nil {
		return nil
	}
	...
	for _, tx := range res.Txs {
		...
		for _, rawEvent := range events {
			e, err := shielderevents.MakeEvent(rawEvent, tx.Height)
			if err != nil {
				return err
			}
			switch event := e.(type) {
			...
			default:
				// even though we only ask for shielder.new-dk-instance events the transactions will also contain other events.
				// Let's ignore them
			}
		}
	}
	...
	for {
		...
		case tx := <-kpr.txs:
			...
			for _, ev := range d.TxResult.Result.Events {
				x, err := shielderevents.MakeEvent(ev, d.Height)
...
func (kpr *Keyper) dispatchEventToBatch(batchIndex uint64, ev shielderevents.IEvent) {
	kpr.Lock()
	defer kpr.Unlock()
...
func (kpr *Keyper) dispatchEventToDKG(eon uint64, ev shielderevents.IEvent) {
	kpr.Lock()
	defer kpr.Unlock()

	d, ok := kpr.dkg[eon]
	if ok {
		d.dispatchShielderEvent(ev)
	} else {
		log.Printf("dkg is not active, cannot dispatch %+v", ev)
	}
}

func (kpr *Keyper) dispatchEvent(ev shielderevents.IEvent) {
	switch e := ev.(type) {
...
func (kpr *Keyper) startNewDKGInstance(ev shielderevents.EonStarted) {
	_, ok := kpr.dkg[ev.Eon]
	if ok {
...
func (kpr *Keyper) handleCheckInEvent(ev shielderevents.CheckIn) {
	kpr.Lock()
	defer kpr.Unlock()
