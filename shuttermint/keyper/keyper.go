...
func (kpr *Keyper) fetchCurrentDKG(ctx context.Context) error {
	...
	for _, tx := range res.Txs {
		...
		for _, rawEvent := range events {
			e, err := shielderevents.MakeEvent(rawEvent)
			if err != nil {
				return err
			}
			switch event := e.(type) {
...
func (kpr *Keyper) startNewDKGInstance(ev shielderevents.EonStartedEvent) {
	_, ok := kpr.dkg[ev.Eon]
	if ok {
...
func (kpr *Keyper) handleCheckInEvent(ev shielderevents.CheckInEvent) {
	kpr.Lock()
	defer kpr.Unlock()
