...
func (runenv *RunEnv) handleShielderMessages(ctx context.Context) {
	for {
		select {
		case id := <-runenv.shuttermintMessages:
			a := runenv.PendingActions.GetAction(id).(*SendShielderMessage)
			var err error
			for {
...
func (runenv *RunEnv) StartBackgroundTasks(ctx context.Context, g *errgroup.Group) {
	...
	g.Go(func() error {
		runenv.handleShielderMessages(ctx)
		return nil
	})
